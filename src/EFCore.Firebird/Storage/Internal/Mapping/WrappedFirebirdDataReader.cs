...
{
    /// <summary>
    /// <see cref="WrappedFirebirdDataReader"/> wraps <see cref="DbDataReader" /> and enhances <see cref="GetFieldValue{T}"/> to
    /// use reflection to cast from <c>byte[]</c> if the regular method fails; this allows JSON objects to be deserialized.
    /// </summary>
    public class WrappedFirebirdDataReader : DbDataReader
    {

        private DbDataReader _reader;
        private bool _disposed;

        internal WrappedFirebirdDataReader(DbDataReader reader)
        {
            _reader = reader;
        }
        ...
        private void CloseReader()
        {
            if (_reader != null)
            {
                // dispose the underlying Firebird data reader
                _reader.Dispose();
                _reader = null;
            }
        }
        ...
        {
            if (_reader == null)
                throw new ObjectDisposedException(nameof(WrappedFirebirdDataReader));
            return _reader;
        }
    }
}
